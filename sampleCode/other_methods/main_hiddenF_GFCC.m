...
for nSpk = 1:length(trainList)
    ...
    for i=1:length(currSpkList)
        ...
        currFile = strrep(currFile,'female','FEMALE');
        currFile = strrep(currFile,'male','MALE');
        display(currFile)
        ...
        num_windows = size(currFeatures,1);
...
for i = 1:NumTestData
    ...
    col = size(currFeatures,1);
    hiddenF = tansig(repmat(bias,1,col)+weights*currFeatures');
