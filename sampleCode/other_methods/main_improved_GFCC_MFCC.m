...
for nSpk = 1:length(trainList)
    ...
    for i=12:length(currSpkList)
        ...
        currFeatures1 = feature_warped_gfcc';

        
        % MFCC FEATURES (for comparison)
        mfcc = get_MFCC(snd,Fs, ftrParams);
        cmvn_mfcc = cmvn(mfcc',true);
        currFeatures2 = cmvn_mfcc';
        
        min_size = min(size(currFeatures1,1), size(currFeatures2,1));
        currFeatures = [currFeatures1(1:min_size,:), currFeatures2(1:min_size,:)];
